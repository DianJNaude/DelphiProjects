...
type
  ...
  procedure ClearAll;
   ...
   function ClientIDExist(sIDQ:string):boolean;
    { Public declarations }
...
//Home button
rgpPS.ItemIndex := -1;
rgpAnimalQuerries.ItemIndex := -1 ;
rgpClientQ.ItemIndex := -1;
...
///Querry button
 ...
 cmbServiceProducts.TextHint := 'Product/service' ;
...
function TfrmAdmin.ClientIDExist(sIDQ: string): boolean;
begin
      dmDataA.tblClient.First;
   while not dmDataA.tblClient.Eof do
    begin
      if inttostr(dmDataA.tblClient['ClientID']) = sIDQ then
       begin
        result := True ;
        bBegin := True;
       end;
...
frmLogin.ActivateDatabase;
sIDCCC :=  edtID.Text;
if ClientIDExist(sIDCCC) = True then
begin
if bBegin = True then
 begin
 redData.Clear;
...
begin
  ...
  begin
    ...
    if cmbProvince.ItemIndex = -1 then
  begin
    ShowMessage('Select a province');
    bExucute := false ;
...
//Checks whether an id exist
   ...
   while not dmDataA.tblAnimals.Eof do
    begin
      if inttostr(dmDataA.tblAnimals['AnimalID']) = sIDV then
       begin
        result := True ;
       end;
    dmDataA.tblAnimals.Next
...
rgpPS.ItemIndex := -1 ;
rgpClientQ.ItemIndex := -1 ;
cmbServiceProducts.ItemIndex := -1;
...
cmbServiceProducts.ItemIndex := -1 ;
 ...
 if iNum = 0 then
 begin
  sOut := 'select tblClient.ClientID, Name, Surname, Company, phone,  cCur(sum(Total)) as [Total amount spent(R)] from tblTransaction, tblclient '  ;
...
iX := cmbServiceProducts.ItemIndex ;
 ...
 begin
  ...
  if iNum = 1 then
  begin
  sOut := 'select ProductID, type, description, cCur(Price) as [Price(R)] from tblProducts order by price desc'  ;
  end;
 ...
 if iNum = 1 then
  begin
  sOut := 'select ServiceID, description, cCur(Price) as [Price(R)] from tblService order by price desc'  ;
  end;
...
procedure TfrmAdmin.ValidateID;
var
sTemp:string;
i, iError:integer;
rX :real;
